...
import { adminMediaItemSchema, variantSchema } from '../../lib/models/route-model';
...
const variantPayloadSchema = variantSchema.extend({
...
/**
 * Create route save handlers. Returns a fresh instance per request
 * to safely encapsulate sharedKeysData from concurrent request contamination.
 */
export function createRouteHandlers(sharedKeysData: Record<string, Array<{ type: string; slug: string }>> = {}): SaveHandlers<RouteUpdate, RouteBuildResult> & WithSlugValidation & WithAfterCommit<RouteBuildResult> {
  return {
...
export async function POST({ params, request, locals }: APIContext) {
  ...
  return saveContent(request, locals, params, 'routes', createRouteHandlers(sharedKeysData));
}
