...
async function handleRequest(locals: APIContext['locals'], url: URL, params: APIContext['params'], forceSync: boolean) {
  ...
  try {
    ...
    if (ctx) {
      if (forceSync) {
        await deleteContentDailyForSlug(database, CITY, 'event', slug, startStr, endStr);
    ...
    const [eng, daily, reactionsByTypeMap] = await Promise.all([
      queryContentEngagement(database, CITY, 'event', slug),
      queryContentTimeSeries(database, CITY, 'event', slug, startStr, endStr),
