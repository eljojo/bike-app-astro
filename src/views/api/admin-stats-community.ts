    ...
    if (apiKey) {
      ...
      if (forceSync) {
        await database.delete(contentPageMetrics)
          ...
          .run();
        await syncPageMetrics(database, { ...syncOpts, contentType: 'organizer', contentSlug: slug });
      } else {
        await ensurePageDailyData(database, syncOpts, 'organizer', slug, startStr, endStr);
      }
    }
