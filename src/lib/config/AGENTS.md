...
| File | Role |
|------|------|
| `config.ts` | Exports `CONTENT_DIR`, `CITY`, `cityDir` from `process.env`. The source of truth for the content directory path |
...
- **`config.ts` vs `city-config.ts`**: `config.ts` is pure env vars (path constants). `city-config.ts` reads YAML and provides domain-specific settings. Don't confuse them.
