...
| `feed-cache.adapter-kv.ts` | Cloudflare KV adapter — reuses the `TILE_CACHE` binding with a `calfeed:feed:v2:` key prefix |
| `feed-cache.adapter-local.ts` | Local filesystem adapter — `.data/calendar-feed-cache/` with JSON + sidecar `.meta` |

## Gotchas

- **Binding reuse.** The KV adapter binds to `TILE_CACHE`, not a dedicated binding. Distinct key prefix (`calfeed:feed:v2:`) prevents collisions with tile entries.
