...
import { buildSingleMediaKeyChanges, computeMediaKeyDiff, buildMediaKeyChanges, buildCommitTrailer, mergeFrontmatter, loadExistingMedia } from '../../src/lib/content/save-helpers.server';
import { CITY } from '../../src/lib/config/config';

describe('buildSingleMediaKeyChanges', () => {
  it('returns empty array when keys are the same', () => {
    expect(buildSingleMediaKeyChanges('abc', 'abc', 'event', 'my-event')).toEqual([]);
  });

  it('returns add when new key replaces undefined', () => {
    const changes = buildSingleMediaKeyChanges(undefined, 'new-key', 'event', 'my-event');
  ...
  it('returns remove when old key replaced by undefined', () => {
    const changes = buildSingleMediaKeyChanges('old-key', undefined, 'place', 'my-place');
    expect(changes).toEqual([
  ...
  it('returns both remove and add when keys differ', () => {
    const changes = buildSingleMediaKeyChanges('old', 'new', 'event', 'evt');
