...
describe('extractPhotoMetadata', () => {
  it('returns null for a PNG buffer (no EXIF)', () => {
    ...
    const result = extractPhotoMetadata(png.buffer);
    expect(result).toBeNull();
  });

  it('returns null for an empty buffer', () => {
    const result = extractPhotoMetadata(new ArrayBuffer(0));
    expect(result).toBeNull();
  });
  ...
  it('returns null for a JPEG without GPS EXIF data', () => {
    ...
    const result = extractPhotoMetadata(jpeg.buffer);
    expect(result).toBeNull();
  });
});

describe('extractGpsCoordinates (deprecated alias)', () => {
  it('returns null for non-JPEG', () => {
    const result = extractGpsCoordinates(new ArrayBuffer(0));
    expect(result).toBeNull();
  });

  it('returns null for a JPEG without GPS', () => {
    const jpeg = Buffer.from([0xff, 0xd8, 0xff, 0xd9]);
