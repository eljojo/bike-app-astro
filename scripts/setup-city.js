...
function deployStub(wranglerEnv) {
  ...
  // A previous build leaves dist/server/wrangler.json — a redirect config
  // generated by the Astro Cloudflare adapter. It defines bindings like
  // SESSION KV without IDs, which makes wrangler try to provision them
  // (and fail because they already exist). Move it aside so wrangler uses
  // the base wrangler.jsonc instead.
  const redirectConfig = resolve(ROOT, 'dist', 'server', 'wrangler.json');
  const redirectBackup = redirectConfig + '.bak';
  const hadRedirect = existsSync(redirectConfig);
  if (hadRedirect) {
    renameSync(redirectConfig, redirectBackup);
  }

  // Base wrangler.jsonc has assets.directory = "./dist" — ensure it exists
  const distDir = resolve(ROOT, 'dist');
  const distCreated = !existsSync(distDir);
  if (distCreated) mkdirSync(distDir, { recursive: true });
  ...
  try {
    ...
    if (hadRedirect) renameSync(redirectBackup, redirectConfig);
    if (distCreated) rmSync(distDir, { recursive: true, force: true });
  }
}
