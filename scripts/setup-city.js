...
function deployStub(wranglerEnv) {
  ...
  // Deploy from a clean temp directory to avoid wrangler state conflicts.
  // The project directory has .wrangler/deploy/config.json and
  // dist/server/wrangler.json from previous builds — these interfere
  // with deploying a new environment.
  const tmpDir = resolve(tmpdir(), `setup-city-${wranglerEnv}-${Date.now()}`);
  mkdirSync(tmpDir, { recursive: true });
  mkdirSync(resolve(tmpDir, 'dist')); // assets.directory

  // Copy wrangler config — strip assets.directory to just "./dist"
  const wranglerConfig = readWranglerConfig();
  wranglerConfig.assets = { ...wranglerConfig.assets, directory: './dist' };
  writeFileSync(resolve(tmpDir, 'wrangler.jsonc'), JSON.stringify(wranglerConfig, null, 2));

  // Stub worker
  const stubPath = resolve(tmpDir, 'stub.mjs');
  writeFileSync(stubPath, 'export default { fetch() { return new Response("Setting up — check back soon!"); } };\n');

  try {
    safeExec(
      `${wranglerCmd()} deploy stub.mjs --config wrangler.jsonc --env ${wranglerEnv}`,
